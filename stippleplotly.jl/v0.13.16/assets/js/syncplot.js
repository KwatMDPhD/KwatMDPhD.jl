function watchPlots(model, observe = true, parentSelector = "") {
  if (parentSelector != "") {
    ...
    plotNodes.forEach(function (gd) {
      watchPlot(gd, model);
    });
  }

  if (observe) {
    sentinel.on(".js-plotly-plot", function (gd) {
      window.watchPlot(gd, model);
    });
  }
}
...
function watchPlot(gd, model) {
  ...
  syncList.forEach(function (c) {
    watchGraphDiv(gd, model, c.match(/sync_(.*)/)[1]);
  });
}
...
function watchGraphDiv(gd, model, prefix) {
  ...
  gd.on("plotly_selected", function (data) {
    ...
    if (!filteredEventData.isNil) {
      model[prefix + "_selected"] = filteredEventData.out;
    }
  });
  ...
  gd.on("plotly_hover", function (data) {
    ...
    if (!filteredEventData.isNil) {
      model[prefix + "_hover"] = filteredEventData.out;
    }
  });
  ...
  gd.on("plotly_click", function (data) {
    ...
    if (!filteredEventData.isNil) {
      model[prefix + "_click"] = filteredEventData.out;
    }
  });
  ...
  gd.on("plotly_relayout", function (data) {
    ...
    if (!filteredEventData.isNil) {
      model[prefix + "_relayout"] = filteredEventData.out;
    }
  });
