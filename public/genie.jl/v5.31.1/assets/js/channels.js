...
Genie.Settings = {
  ...
  webchannels_alert_overlay: false,
  websockets_host: "127.0.0.1",
  webchannels_show_alert: true,
  webthreads_js_file: "webthreads.js",
  webchannels_base64_marker: "base64:",
  webchannels_unsubscribe_channel: "unsubscribe",
  webthreads_default_route: window.CHANNEL,
  ...
  webchannels_default_route: window.CHANNEL,
  ...
  const waitForOpenConnection = () => {
    return new Promise((resolve, reject) => {
      const maxNumberOfAttempts =
        Genie.Settings.webchannels_connection_attempts;
      ...
      const interval = setInterval(() => {
        if (currentAttempt > maxNumberOfAttempts - 1) {
          ...
          reject(
            new Error("Maximum number of attempts exceeded: Message not sent."),
          );
...
function displayAlert(
  content = "Can not reach the server. Trying to reconnect...",
) {
  if (
    document.getElementById(wsconnectionalert_elemid) !== null ||
    wsconnectionalert_triggered
  )
    return;
  ...
  elem.style.cssText =
    "position:fixed;top:0;width:100%;z-index:100;background:#e63946;color:#f1faee;text-align:center;";
  elem.style.height = "1.8em";
  elem.innerHTML = content;

  let elemspacer = document.createElement("div");
  elemspacer.id = wsconnectionalert_elemid + "spacer";
  elemspacer.style.height = Genie.Settings.webchannels_alert_overlay
    ? 0
    : elem.style.height;
...
function newSocketConnection(host = Genie.Settings.websockets_exposed_host) {
  let ws = new WebSocket(
    Genie.Settings.websockets_protocol +
      "//" +
      host +
      (Genie.Settings.websockets_exposed_port > 0
        ? ":" + Genie.Settings.websockets_exposed_port
        : "") +
      ((Genie.Settings.base_path.trim() === "" ||
      Genie.Settings.base_path.startsWith("/")
        ? ""
        : "/") +
        Genie.Settings.base_path) +
      ((Genie.Settings.websockets_base_path.trim() === "" ||
      Genie.Settings.websockets_base_path.startsWith("/")
        ? ""
        : "/") +
        Genie.Settings.websockets_base_path),
  );
...
Genie.Revivers = {};
Genie.Revivers.pipeRevivers = (revivers) => (key, value) =>
...
Genie.WebChannels.messageHandlers.push((event) => {
  try {
    ...
    if (ed.startsWith(Genie.Settings.webchannels_base64_marker)) {
      ed = atob(
        ed.substring(Genie.Settings.webchannels_base64_marker.length).trim(),
      );
    }
    ...
    if (ed.startsWith("{") && ed.endsWith("}")) {
      ...
      return Function(
        '"use strict";return (' +
          ed.substring(Genie.Settings.webchannels_eval_command.length).trim() +
          ")",
      )();
...
function parse_payload(json_data) {
  if (isDev()) {
    console.info(
      "Overwrite window.parse_payload to handle messages from the server",
    );
...
function subscribe(trial = 1) {
  if (
    Genie.WebChannels.socket.readyState &&
...
function unsubscribe() {
  Genie.WebChannels.sendMessageTo(
    window.Genie.Settings.webchannels_default_route,
    window.Genie.Settings.webchannels_unsubscribe_channel,
  );
