const watcherMixin = {
  methods: {
    $withoutWatchers: function (cb, filter) {
      ...
      if (typeof filter == "string") {
        this._watchers.forEach((w) => {
          if (w.expression == filter) {
            ww.push(w);
          }
        });
      } else {
        // if it is a true regex
        this._watchers.forEach((w) => {
          if (w.expression.match(filter)) {
            ww.push(w);
          }
        });
      }

      const watchers = ww.map((watcher) => ({
        cb: watcher.cb,
        sync: watcher.sync,
      }));
    ...
    push: function (field) {
      Genie.WebChannels.sendMessageTo(CHANNEL, "watchers", {
        payload: {
          field: field,
          newval: this[field],
          oldval: null,
        ...
        for (var key in obj) {
          if (
            ...
            if (obj[key] != null && obj[key].jsfunction) {
              obj[key] = Function(
                obj[key].jsfunction.arguments,
                obj[key].jsfunction.body,
              );
            }
          }
        }
...
const eventMixin = {
  methods: {
    handle_event: function (event_data, event_handler, mode) {
      ...
      console.debug(
        "event: " + JSON.stringify(event_data) + ":" + event_handler,
      );
      if (mode == "addclient") {
        event_data._addclient = true;
      }
